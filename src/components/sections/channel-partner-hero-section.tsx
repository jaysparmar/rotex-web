...
export function ChannelPartnerHeroSection({
  ...
  return (
    ...
    <section className="relative bg-stone-900 lg:min-h-0 lg:h-170 pt-28 pb-4 lg:pb-20 overflow-hidden">
      {/*
      ...
      */}
      ...
      <div className="container relative z-10 flex flex-col lg:h-full lg:justify-between">
        {/* Breadcrumb */}
        ...
        <div className="flex flex-col gap-5 mt-10 lg:mt-0 lg:w-171.25">
          ...
          <Link
            href={cta.href}
            className="inline-flex w-fit h-10 lg:h-auto items-center justify-center gap-1.5 px-6 lg:py-3.5 whitespace-nowrap rounded-[47px] bg-white lg:bg-stone-100 outline outline-[0.5px] -outline-offset-[0.5px] outline-[#EF3E23] lg:outline-0 text-[#EF3E23] text-sm font-bold font-montserrat uppercase leading-5 shadow-[0px_13px_7.8px_-12px_rgba(0,0,0,0.25)] hover:bg-primary hover:text-white hover:outline-primary transition-colors duration-200"
          >
            {cta.label}
          </Link>

          {/* Mobile artwork — same technique as the homepage hero's HexSlider:
              in-flow (not absolute), a small top margin, negative right margin
              to bleed past the container's own padding, and >100% width so it
              extends further right than that. */}
          <div className="relative mt-2 -mr-8 lg:hidden">
            <div className="w-[112%]">
              <HexFrame
                src={mobilePhoto}
                alt="Rotex channel partners shaking hands"
