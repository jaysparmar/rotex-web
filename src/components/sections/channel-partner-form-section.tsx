...
function FormSelect({
  ...
  name: "businessType" | "industriesServed" | "productType";
  placeholder: string;
  options: string[];
  hasError: boolean;
}) {
...
export function ChannelPartnerFormSection({
  ...
  const onSubmit = async (data: FormData) => {
    ...
    body.append("source", "channel-partner");
    ...
    const recaptchaToken = await getToken("channel_partner_application");
    ...
    if (!json.success) {
      ...
      throw new Error(json.error?.message ?? "Failed to submit channel partner application");
    }
  };
    ...
    <section id="form" className="scroll-mt-24 lg:scroll-mt-32 bg-white py-14 lg:py-20">
      <div className="container flex flex-col gap-10 lg:flex-row lg:items-start lg:gap-16">
        ...
        <div className="lg:w-121.75 lg:shrink-0 flex flex-col gap-4">
          <h2 className="text-stone-900 text-2xl lg:text-4xl font-medium font-montserrat leading-8 lg:leading-10">
            {headingPrefix}{" "}
        ...
        >
          ...
          <div className="flex flex-col gap-5 lg:flex-row">
            <Field label="Business Type" error={errors.businessType?.message} className="flex-1" name="businessType">
              <FormSelect control={control} name="businessType" placeholder="Select Business Type" options={businessTypeOptions} hasError={!!errors.businessType} />
            </Field>

            <Field label="Industries Served" error={errors.industriesServed?.message} className="flex-1" name="industriesServed">
              <FormSelect control={control} name="industriesServed" placeholder="Select Industries Served" options={industryOptions} hasError={!!errors.industriesServed} />
            </Field>
          </div>
