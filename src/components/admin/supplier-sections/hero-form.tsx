...
type HeroData = { title: string; description: string; image: string; cta: { label: string; href: string } };
type FormValues = {
  enabled: boolean;
  title: string;
  description: string;
...
export function SupplierHeroForm({
  initialEnabled,
  initialData,
}: {
  ...
  function onSubmit(values: FormValues) {
    ...
    run(async () => {
      try {
        await saveSupplierSection("hero", { enabled, data });
